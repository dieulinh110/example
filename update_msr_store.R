...
msr_store_update <- read_excel("~/MCP MEKONG1 - Sent.xlsx")
...
msr_store_update <- mutate(msr_store_update, 
                           store_add_new = paste_visit(list("SN:", msr_store_update[, 3]), sep = " "))

msr_store_update <- mutate(msr_store_update,
                           store_add_new = paste_visit(list(msr_store_update$store_add_new,
...
msr_store_update <- msr_store_update %>% 
    rename(city_lb_new = `Tỉnh/Thành Phố`)
