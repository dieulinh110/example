...
output$posm_xls <- downloadHandler(
    filename = paste0("MSR_POSM_", format(Sys.time(), "%d%m%Y_%H%M%S"), ".xlsx"),
    content = function(file) {
        ## make workbook
        wb <- create_excel_file(re_sup_posm_re(),
                                sheet_name = "posm",
                                sheet_title = c("Posm"))
...
output$inventory_xls <- downloadHandler(
    filename = paste0("MSR_INVENTORY_", format(Sys.time(), "%d%m%Y_%H%M%S"), ".xlsx"),
    content = function(file) {
        ## make workbook
        wb <- create_excel_file(re_sup_inventory_re(),
                                sheet_name = "inventory",
                                sheet_title = c("Inventory"))
        ## save workbook to a file
        saveWorkbook(wb, file)
    }
)
...
output$competitor_xls <- downloadHandler(
    filename = paste0("MSR_COMPETITOR_", format(Sys.time(), "%d%m%Y_%H%M%S"), ".xlsx"),
    content = function(file) {
        ## make workbook
        wb <- create_excel_file(re_sup_competitor_re(),
                                sheet_name = "competitor",
                                sheet_title = c("Competitor"))
        ## save workbook to a file
        saveWorkbook(wb, file)
    }
)
...
output$store_review_xls <- downloadHandler(
    filename = paste0("MSR_STORE_REVIEW_", format(Sys.time(), "%d%m%Y_%H%M%S"), ".xlsx"),
    content = function(file) {
        ## make workbook
        wb <- create_excel_file(re_sup_survey_store(),
                                sheet_name = "store review",
                                sheet_title = c("Store Review"))
        ## save workbook to a file
        saveWorkbook(wb, file)
    }
)
...
output$survey_game_xls <- downloadHandler(
    filename = paste0("MSR_SURVEY_GAME_", format(Sys.time(), "%d%m%Y_%H%M%S"), ".xlsx"),
    content = function(file) {
        ## make workbook
        wb <- create_excel_file(re_sup_survey_game(),
                                sheet_name = "survey game",
                                sheet_title = c("Survey Game"))
        ## save workbook to a file
        saveWorkbook(wb, file)
    }
)
...
fluidRow(
  column(
    width = 2,
    downloadButton(outputId = 'inventory_xls', label = 'xlsx')
  )
)
...
fluidRow(
  column(
    width = 2,
    downloadButton(outputId = 'competitor_xls', label = 'xlsx')
  )
)
...
fluidRow(
  column(
    width = 2,
    downloadButton(outputId = 'store_review_xls', label = 'xlsx')
  )
)
...
fluidRow(
  column(
    width = 2,
    downloadButton(outputId = 'survey_game_xls', label = 'xlsx')
  )
)
