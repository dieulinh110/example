...
if (any(!msr_store_update$city_id_new %in% province$dgw_city_id)) {
    city_exist <- msr_store_update$city_id_new[!msr_store_update$city_id_new %in% province$dgw_city_id]
    stop(paste0('Dữ liệu Mã Tỉnh/Thành phố "', city_exist, '" không có trong hệ thống.\n'), call. = FALSE)
}

if (any(!msr_store_update$salerep_id_new %in% dgw_staff_id$dgw_staff_id)) {
    staff_exist <- msr_store_update$salerep_id_new[!msr_store_update$salerep_id_new %in% dgw_staff_id$dgw_staff_id]
    stop(paste0('Dữ liệu Mã NVBH (Sales Rep) "', staff_exist, '" không có trong hệ thống.\n'), call. = FALSE)
}
...
msr_store_update <- left_join(msr_store_update, 
                              select(province, city_id_new = dgw_city_id, dgw_city_lb, 
                                     city_id_final = city_id, city_lb_final = city_lb), 
                              by = "city_id_new")
msr_store_update$city_id_new <- msr_store_update$city_id_final
msr_store_update$city_lb_new <- msr_store_update$city_lb_final

## Create salerep_id

msr_store_update <- left_join(msr_store_update, dgw_staff_id, 
                              by = c("salerep_id_new" = "dgw_staff_id"))
...
msr_store_not_check$update_time <- as.Date(msr_store_not_check$update_time, "%Y-%m-%d")	#%>% as.POSIXct()		  
...
replace_data <- function(data) {
    ...
    data$update_time <- as.Date(data$update_time, "%Y-%m-%d") #%>% as.POSIXct()
    data
}
...
msr_store_off$update_time <- as.Date(msr_store_off$update_time, "%Y-%m-%d") #%>% as.POSIXct()
...
msr_store_test$update_time <- as.Date(msr_store_test$update_time, "%Y-%m-%d") #%>% as.POSIXct()
...
final <- msr_store_final %>% 
    select(store_id, store_lb,
           ...
           salerep_id, salerep_lb, asm_id, asm_lb, rsm_id, rsm_lb, update_time)
